...
const { createUser,updateUrl,getTradeUrl,updateInvCall,getInvCall } = require('./dbServer');
// Let's set a port
const  port = 3001;
// Spin up the server
const app = express();
...
passport.use(new SteamStrategy({
    returnURL: apiUrl+':3001/api/auth/steam/return',
    realm: apiUrl+':3001/',
...
app.get("/api/settings/getTradeUrl", isAuthenticated, function (req, res) {
    ...
    getTradeUrl(req.user.id, (result, error) => {
        if (error) {
            ...
            res.status(404).json({ body: "K�ytt�j�n url ei l�ydy" });
        } else {
...
app.get("/api/settings/getInvRefresh",isAuthenticated,function (req, res) {
    ...
    getInvCall(req.user.id, (result, error) => {
        ...
        if (error) {
            ...
            res.status(404).json({ body: "K�ytt�j�n tietoja ei l�ydy" });
        } else {
