...
const { createTrade, createTrustlyRow, getUserTrades } = require('./dbServer');
...
*/
                ...
                Trade.checkUserStatus(useBot, tradeToken, steamId,async(result) => {
                    ...
                    if (!result) {
                        ...
                        //db
                        createTrade(infObject, async (dbResult) => {
                          console.log(dbResult);
                            ...
                            if (!dbResult) {
                                ...
                                const trustlyInfo={
                                    ...
                                    authorizer:steamId,
                                    authorizerIP:socket.request.connection.remoteAddress,
                                    orderStatus:0,

                                }
                                 try {
                                const trustlyRowResult = await createTrustlyRow(trustlyInfo);
                                console.log(trustlyRowResult);
                                 }catch(error){
                                    console.log(error);
                                    return;
                                 }
        ...
        if (steamStatus) {
            ...
            if (cacheData) {
                ...
                try {
                    ...
                    const errorMsg = "ErrorCode: " + errorBody.statusCode +"  |  ErrorMessage: " + errorBody.message;

                    Trade.writeLogs("system",errorMsg);
                    socket.emit("user inv", { error: "invError", items: null });
                }
